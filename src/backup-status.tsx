...
export function BackupStatus(){
 ...
 useEffect(()=>{let active=true;api<{lastSuccess:string|null;lastError:string|null;running:boolean}>('/backup-status').then(v=>{if(active)setState(v)}).catch(()=>{if(active)setError('Zaxira holatini yuklab bo‘lmadi. Sahifani yangilang.')});return()=>{active=false}},[]);
