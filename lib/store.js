...
export function AppProvider({ children }) {
  ...
  // mijoz o'z topshirig'ini bajardi deb belgilaydi
  const taskDone = async (id) => {
    patch((d) => { const t = d.tasks.find((x) => x.id === id); if (t) t.status = "bajarildi"; });
    if (serverMode && session?.role === "client") {
      try {
        const r = await fetch("/api/client/task-done", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ taskId: id }),
        });
        if (!r.ok) toast("Serverga yetkazilmadi, qayta urinib ko'ring");
      } catch {
        toast("Server bilan aloqa yo'q");
      }
    }
