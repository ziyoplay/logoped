...
if(restored.prepare("SELECT 1 FROM sqlite_master WHERE type='table' AND name='patient_questions'").get())restored.exec("UPDATE patient_questions SET state='failed' WHERE state NOT IN ('new','sent')");}finally{restored.close();}
