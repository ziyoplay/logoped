...
export async function readSnapshot(filename){const archive=JSON.parse(await readFile(filename,'utf8'));if(archive.format!=='nutq-postgres-backup-v1'||![1,2,3,4].includes(archive.data?.schemaVersion)||archive.sha256!==digest(archive.data))throw Error('Zaxira formati yoki nazorat xeshi noto‘g‘ri.');if(archive.data.schemaVersion===1){archive.data.tables.client_accounts=[];archive.data.tables.patient_exercises=[];}if(archive.data.schemaVersion<3){archive.data.tables.patient_telegram=[];archive.data.tables.patient_videos=[];}if(archive.data.schemaVersion<4)archive.data.tables.patient_questions=[];for(const table of snapshotTables)if(!Array.isArray(archive.data.tables[table]))throw Error('Zaxira jadvali yetishmaydi: '+table);return archive.data;}
