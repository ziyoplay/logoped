...
export async function restoreSnapshot(db,data){
 ...
 await db.transaction(async()=>{
  ...
  for(const table of snapshotTables){if(['sessions','invitations'].includes(table))continue;
   ...
   for(const row of data.tables[table]){const keys=Object.keys(row).filter(k=>columns.includes(k));if(!keys.length)throw Error('Zaxira qatori noto‘g‘ri.');await db.prepare('INSERT INTO '+table+'('+keys.map(k=>'"'+k+'"').join(',')+') VALUES('+keys.map(()=>'?').join(',')+')').run(...keys.map(k=>row[k]));}
  }
 });
}
