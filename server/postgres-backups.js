...
export const snapshotTables=['users','clinics','patients','exercises','appointments','results','sessions','invitations','audit_log','client_accounts','patient_exercises','patient_telegram','patient_videos','patient_questions','calendar_connections','calendar_events','appointment_notifications'];
const digest=value=>createHash('sha256').update(JSON.stringify(value)).digest('hex');
export async function snapshot(db){return db.transaction(async()=>{const tables={};for(const table of snapshotTables)tables[table]=await db.prepare('SELECT * FROM '+table+(['patients','exercises','appointments','results'].includes(table)?' ORDER BY created_order':table==='audit_log'?' ORDER BY id':'')).all();return {schemaVersion:5,createdAt:new Date().toISOString(),tables};},{isolation:'repeatable read'});}
...
export async function readSnapshot(filename){const archive=JSON.parse(await readFile(filename,'utf8'));if(archive.format!=='nutq-postgres-backup-v1'||![1,2,3,4,5].includes(archive.data?.schemaVersion)||archive.sha256!==digest(archive.data))throw Error('Zaxira formati yoki nazorat xeshi noto‘g‘ri.');if(archive.data.schemaVersion===1){archive.data.tables.client_accounts=[];archive.data.tables.patient_exercises=[];}if(archive.data.schemaVersion<3){archive.data.tables.patient_telegram=[];archive.data.tables.patient_videos=[];}if(archive.data.schemaVersion<4)archive.data.tables.patient_questions=[];if(archive.data.schemaVersion<5)for(const t of ['calendar_connections','calendar_events','appointment_notifications'])archive.data.tables[t]=[];for(const table of snapshotTables)if(!Array.isArray(archive.data.tables[table]))throw Error('Zaxira jadvali yetishmaydi: '+table);return archive.data;}
export async function restoreSnapshot(db,data){
 ...
 await db.transaction(async()=>{
  ...
  for(const table of snapshotTables){if(['sessions','invitations','patient_telegram','calendar_connections','calendar_events','appointment_notifications'].includes(table))continue;
   ...
   for(const original of (data.tables[table]||[])){const row=table==='patient_questions'?{...original,state:['new','sent'].includes(original.state)?original.state:'failed'}:table==='patient_videos'?{...original,state:original.state==='sent'?'sent':'failed',error:original.state==='sent'?'':'Zaxiradan tiklangan. Telegramni tekshirib, qayta yuboring.'}:original;const keys=Object.keys(row).filter(k=>columns.includes(k));if(!keys.length)throw Error('Zaxira qatori noto‘g‘ri.');await db.prepare('INSERT INTO '+table+'('+keys.map(k=>'"'+k+'"').join(',')+') VALUES('+keys.map(()=>'?').join(',')+')').run(...keys.map(k=>row[k]));}
  }
 });
}
