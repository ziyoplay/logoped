...
test('Personal workspace shows backup status without team or therapist controls',async({page})=>{
 await page.goto('/#kirish');await page.getByRole('button',{name:'Hisob yarating',exact:true}).click();await expect(page.getByLabel('Markaz taklif kodi (xodim uchun)')).toHaveCount(0);await page.getByRole('button',{name:'Namuna bilan ko‘rish'}).click();
