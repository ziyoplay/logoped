...
test('Default personal mode isolates accounts and disables team invitations',async()=>{
 const {app,db}=createApp({filename:':memory:'});const server=app.listen(0,'127.0.0.1');await new Promise(r=>server.once('listening',r));const base='http://127.0.0.1:'+server.address().port;
