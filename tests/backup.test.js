...
test('Online backup preserves records; restore clears sessions and refuses overwrite',async()=>{
 ...
 db.prepare('INSERT INTO calendar_connections(owner_id,credentials) VALUES(?,?)').run('u','encrypted-test-fixture');
 db.prepare('INSERT INTO calendar_events(owner_id,event_id,appointment_id) VALUES(?,?,?)').run('u','test-event','test-appointment');
 ...
 execFileSync(process.execPath,['scripts/restore.js',source,target]);const copy=new DatabaseSync(target);try{assert.equal(copy.prepare('SELECT count(*) AS n FROM users').get().n,1);assert.equal(copy.prepare('SELECT count(*) AS n FROM sessions').get().n,0);assert.equal(copy.prepare('SELECT count(*) AS n FROM calendar_connections').get().n,0);assert.equal(copy.prepare('SELECT count(*) AS n FROM calendar_events').get().n,0);assert.equal(copy.prepare('PRAGMA integrity_check').get().integrity_check,'ok');}finally{copy.close();}
